...
function cn_proxy_status_badge_class(?string $s): string {
...
if ($primary !== null) {
    $cnContextBar['senior_line'] = count($linked) === 1
        ...
        : ('Supporting ' . count($linked) . ' seniors · shown below');
...
<main class="main-content">
    ...
    <?php if (isset($_GET['booked'])): ?>
        <div class="alert-cn alert-cn-success mb-4"><?= e('Visit requested for your senior. Their Pal will confirm soon.') ?></div>
    <?php endif; ?>

    <?php if (!$linked): ?>
        <div class="cn-card cn-card-body"><?= e('No linked seniors yet. Complete registration by linking to a senior account first.') ?></div>
    <?php else: ?>
        <div class="cn-card cn-card-body">
            <h1 class="h4 mb-4"><?= e('Visits for your seniors') ?></h1>
            <div class="cn-table-wrap">
                <table class="cn-table mb-0">
                    ...
                    <?php foreach ($rows as $vr): ?>
                        ...
                        ?>
                        <tr>
                            <td><?= e(trim((string) ($nameBySenior[$sidRow] ?? 'Senior'))) ?></td>
                            ...
                            <td><span class="badge-status <?= e(cn_proxy_status_badge_class(isset($vr['status']) ? (string) $vr['status'] : null)) ?>">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$rows): ?>
                        <tr><td colspan="8" style="color:var(--text-secondary);"><?= e('No visits yet.') ?></td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
